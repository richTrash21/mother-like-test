...
class FlxOgmo3Loader
{
	...
	{
		project = OpenFL_Assets.getText(projectData).parseProjectJSON();
		level = OpenFL_Assets.getText(levelData).parseLevelJSON();
	}
	 ...
	 */
	inline public function getLevelValue(value:String):Dynamic
	{
	...
	public function loadTilemap(tileGraphic:FlxTilemapGraphicAsset, tileLayer:String = "tiles", ?tilemap:FlxTilemap):FlxTilemap
	{
		...
		final layer = level.getTileLayer(tileLayer);
		final tileset = project.getTilesetData(layer.tileset);
	...
	{
		var _grid:String;
		final gridLayer = level.getGridLayer(gridLayer);
		final out = new Map<String, Array<FlxPoint>>();
		switch (gridLayer.arrayMode)
		{
			case 0:
				for (i in 0...gridLayer.grid.length)
				{
					_grid = gridLayer.grid[i];
					if (!out.exists(_grid))
						out.set(_grid, []);

					out[_grid].push(FlxPoint.get((i % gridLayer.gridCellsX) * gridLayer.gridCellWidth,
			...
			case 1:
				for (j in 0...gridLayer.grid2D.length)
				{
					...
					{
						_grid = gridLayer.grid2D[j][i];
						if (!out.exists(_grid))
							out.set(_grid, []);

						out[_grid].push(FlxPoint.get(i * gridLayer.gridCellWidth, j * gridLayer.gridCellHeight));
					}
				}
		}
		return out;
	 ...
	 * @param factory		Factory for the FlxSprites for this decal layer. If `null`, default factory is used.
	 */
	public function loadDecals(decalLayer:String = "decals", decalsPath:String, ?factory:(Decal:DecalData, Path:String) -> FlxSprite):FlxGroup
	{
		/*inline*/ function defaultFactory(Decal:DecalData, Path:String):FlxSprite
		{
			final s = new FlxSprite(Decal.x, Decal.y, Path + Decal.texture);
			// s.offset.set(s.width * .5, s.height * .5);
		...
		if (decalsPath != null && decalsPath.length > 0 && !decalsPath.endsWith("/"))
			decalsPath += "/";

		if (factory == null)
			factory = defaultFactory;

		final g = new FlxGroup();
		for (decal in level.getDecalLayer(decalLayer).decals)
			g.add(factory(decal, decalsPath));
	 ...
	 */
	inline static function parseLevelJSON(json:String):LevelData
	{
	 ...
	 */
	inline static function parseProjectJSON(json:String):ProjectData
	{
		return cast Json.parse(json);
	}
	 ...
	 */
	inline static function applyFlagsToTilemapExt(tileFlags:Array<Int>, tilemap:FlxTilemapExt)
	{
		final specialTiles = new Array<FlxTileSpecial>();

		for (i in 0...tileFlags.length)
		{
			final flag = tileFlags[i];
			final specialTile = new FlxTileSpecial(tilemap.getTileByIndex(i), flag & 4 > 0, flag & 2 > 0, 0);
